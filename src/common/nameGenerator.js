	...
	_.each(['first', 'last'], function(position) {
		...
		if (position == 'first') {
			/*
                 ...
                 */
			opt = {
				usePre: { t: 'p', use: Math.random() > 0.2 },
				useCon: { t: 'c', use: Math.random() > 0.2 },
				useSuf: { t: 's', use: Math.random() > 0.2 }
			}
		}
		if (position == 'last') {
			/*
                 ...
                 */
			opt = {
				useFam: { t: 'f', use: Math.random() > 0.1 },
				useSecondFam: { t: 'f', use: Math.random() > 0.9 },
				useCon: { t: 'c', use: Math.random() > 0.2 },
				useSuf: { t: 's', use: Math.random() > 0.2 }
			}
		}
		...
		_.each(opt, function(option) {
			...
			if (option.use) {
				...
				/*
                     ...
                     */
				word += _.find(_.shuffle(list), function() {
					return true
				}).p
			}
		})
